...
lemma p3_center_contradiction (h : Nat.card G = p ^ 4) (pp : p.Prime) (h3_eq : Nat.card (center G) = p ^ 3) :
  ...
  have : Nat.card (G ⧸ center G) = p := by
    calc
      ...
      _ = p := by
        ...
        norm_num
  have cyclic_quotient : IsCyclic (G ⧸ center G) := @isCyclic_of_prime_card _ _ _ ⟨pp⟩ this
  exfalso
...
lemma p4_center (h : Nat.card G = p ^ 4) (pp : p.Prime) (h4_eq : Nat.card (center G) = p ^ 4) :
    ∃ H : Subgroup G, H.IsCommutative ∧ Nat.card H = p ^ 3 := by
  ...
  exact ⟨H, @commGroup_isCommutative _ (@card_center_eq_card_comm_group _ _ _ (G_finite h pp) h h4_eq) H, h_eq⟩
...
lemma p2_card_closure_center_g [Finite G] (h : Nat.card G = p^4) (pp : p.Prime) (h2_eq : Nat.card (center G) = p ^ 2)
  ...
  · have : closure ({g} ∪ center G) = ⊤ := (card_eq_iff_eq_top _).mp (hk_eq ▸ h.symm)
    have : g ∈ center G := mem_center_iff.mpr (fun g_1 ↦ @commgroup_if_top_comm _ _ (this.symm ▸ closure_center_g_iscomm) g_1 g)
    contradiction
...
lemma p2_center (h : Nat.card G = p ^ 4) (pp : p.Prime) (h2_eq : Nat.card (center G) = p ^ 2) :
    ∃ H : Subgroup G, H.IsCommutative ∧ Nat.card H = p ^ 3 := by
  obtain ⟨g, gnin⟩ := @exists_diff_if_card _ _ (G_finite h pp) (center G)
    (h2_eq ▸ h ▸ Nat.pow_lt_pow_of_lt (Prime.one_lt pp) (by norm_num))
  exact ⟨closure ({g} ∪ center G), closure_center_g_iscomm, @p2_card_closure_center_g _ _ _ (G_finite h pp) h pp h2_eq _ gnin⟩
...
lemma p1_p2_card_centr_exists_p3 (h : Nat.card G = p ^ 4) (pp : Nat.Prime p) (h1_eq : Nat.card ↥(center G) = p ^ 1) : ∃ (g' : G), g' ∉ center G ∧ Nat.card (centralizer {g'}) = p ^ 3 := by
  by_contra h'
  push_neg at h'
...
lemma p1_card_centr_p3_comm [Finite G] (h : Nat.card G = p ^ 4) (pp : Nat.Prime p) (h1_eq : Nat.card ↥(center G) = p ^ 1) (g : G) (gnin : g ∉ center G) (h' : Nat.card (centralizer {g}) = p ^ 3) : (centralizer {g}).IsCommutative := by
  ...
  · have : Nat.card (centralizer {g} ⧸ center (centralizer {g})) = p := by
    ...
    have quot_cyclic : IsCyclic (centralizer {g} ⧸ center (centralizer {g})) :=
      @isCyclic_of_prime_card _ _ _ ⟨pp⟩ this
...
lemma p1_center (h : Nat.card G = p ^ 4) (pp : Nat.Prime p) (h1_eq : Nat.card (center G) = p ^ 1) :
  ∃ H : Subgroup G, H.IsCommutative ∧ Nat.card H = p ^ 3 := by
  obtain ⟨g, gnin⟩ := @exists_diff_if_card _ _ (G_finite h pp) (center G) (h1_eq ▸ h ▸ Nat.pow_lt_pow_of_lt (Prime.one_lt pp) (by norm_num))
  ...
  · obtain ⟨g', g'nin, h3_eq⟩ := p1_p2_card_centr_exists_p3 h pp h1_eq
    exact ⟨centralizer {g'}, @p1_card_centr_p3_comm _ _ _ (G_finite h pp) h pp h1_eq g' g'nin h3_eq, h3_eq⟩
  · exact ⟨centralizer {g}, @p1_card_centr_p3_comm _ _ _ (G_finite h pp) h pp h1_eq g gnin h3_eq, h3_eq⟩
...
theorem comm_p3_in_p4 (h : Nat.card G = p ^ 4) (pp : p.Prime) : ∃ H : Subgroup G, H.IsCommutative ∧ Nat.card H = p ^ 3 := by
  rcases (@IsPGroup.card_center_eq_prime_pow _ _ _ ⟨pp⟩ _ h (by norm_num)) with ⟨k, _, hk_eq⟩
  have : k ≤ 4 := (pow_le_pow_iff_right₀ (Prime.one_lt pp)).mp (hk_eq ▸ h ▸ @card_le_card_group _ _ (center G) (G_finite h pp))
  interval_cases k
